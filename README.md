...
正式な読みと、区切りやルビから通常表記へ切り替わる位置を開始点とした別名について、3文字以上の前方部分をすべて登録します。同じ読みになるカードは削除せず、複数の変換候補として収録します。
...
3文字以上の前方読みを辞書へ実際に登録しているため、`＠りとる`や`＠おろかな`を入力して通常のSpace変換を実行できます。数字別名は`＠３`のように1文字から変換できます。
