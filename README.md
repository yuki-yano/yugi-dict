...
＠ざいほう → 《罪宝の囁き》など
＠りとる → 《Ｓ：Ｐリトルナイト》など
＠ういっち → 《ウィッチクラフト・バイスマスター》《黒薔薇の魔女》など
＠２２４ → 《カラクリ小町 弐弐四》
```

読みの先頭は、日本語入力中に`@`キーを押したときの全角文字`＠`です。カード名中の空白・中点などの装飾記号は読みから除き、英数字は全角の表記を保ちます。

正式な読みを、yaml-yugiの`name.ja_romaji`にある空白区切りと照合して単語へ分割します。これにより、表記に中点がない`Ｓ：Ｐリトルナイト`も`えすぴー`、`りとる`、`ないと`として扱えます。ローマ字と安全に照合できない数字・英字入りの名称も、ルビ・空白・記号の境界から候補を生成します。

正式な読み、単語の開始位置、ルビ・記号の境界を開始点として、3文字以上かつ単位の終端にある前方読みを登録します。`＠ぶらっく`や`＠ざいほう`は登録しますが、単位の途中で終わる`＠ぶらっ`や`＠ざいほ`、促音で終わる前方読みは登録しません。同じ読みになるカードは削除せず、複数の変換候補として収録します。

「ウィッチ」を含むカードはテーマ名や複合語も含めて`＠うぃっち`と入力揺れの`＠ういっち`へ登録します。「スウィッチ」は別の単語として除外します。
